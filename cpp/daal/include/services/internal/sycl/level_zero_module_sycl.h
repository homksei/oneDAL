...
private:
    explicit ZeModule(cl::sycl::queue & deviceQueue, size_t binarySize, const uint8_t * pBinary, Status & status)
    {
        ...
        DAAL_CHECK_LEVEL_ZERO(
            _zeModuleCreateF(cl::sycl::get_native<cl::sycl::backend::ext_oneapi_level_zero>(deviceQueue.get_context()),
                             cl::sycl::get_native<cl::sycl::backend::ext_oneapi_level_zero>(deviceQueue.get_device()), &desc, &_moduleLevelZero, nullptr),
            status);
    }
