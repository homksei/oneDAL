...
def configure_cc_toolchain_win(repo_ctx, reqs):
    ...
    repo_ctx.template(
        ...
        substitutions = {
            ...
            "%{tool_bin_path}": paths.dirname(tools.cc),
            "%{common_options}": get_starlark_list(common_options),
            "%{pedantic_options}": get_starlark_list(pedantic_options),
            "%{dpcc_common_options}": get_starlark_list(dpcc_common_options),
            "%{dpcc_pedantic_options}": get_starlark_list(dpcc_pedantic_options),
            "%{cpu_options}": get_starlark_list_dict(cpu_options),
            "%{dpcc_cpu_options}": get_starlark_list_dict(dpcc_cpu_options),
            "%{system_lib_directories}": get_starlark_list(system_lib_directories),
        },
        executable = False,
    )
